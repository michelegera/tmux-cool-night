...
# shellcheck source=src/palettes/cool.sh
. "$CURRENT_DIR/palettes/$theme_variation.sh"
	...
	for plugin in "${plugins[@]}"; do
		...
		if [ ! -f "${CURRENT_DIR}/plugin/${plugin}.sh" ]; then
			...
			if [ "$plugin" == "datetime" ] || [ "$plugin" == "battery" ]; then
				plugin_output="#[fg=${PALETTE[white]},bg=${accent_color}]${plugin_execution_string}#[none]"
			else
